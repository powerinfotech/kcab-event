...
/**
 * LoginUserArgumentResolver - {@code @PowerSession} 어노테이션 기반 로그인 사용자 주입 리졸버
 *
 * <p>Controller 메서드 파라미터에 {@link PowerSession} 어노테이션이 붙은 {@link LoginUser} 타입이
 ...
 *     public ResponseEntity<?> getMyInfo({@code @PowerSession} LoginUser loginUser) {
 ...
 * <h3>동작 조건</h3>
 * <ul>
 *   <li>파라미터에 {@code @PowerSession} 어노테이션이 있어야 한다.</li>
...
public class LoginUserArgumentResolver implements HandlerMethodArgumentResolver {
     ...
     * @return {@code @PowerSession} 어노테이션과 {@link LoginUser} 타입을 모두 만족하면 {@code true}
     */
    @Override
    public boolean supportsParameter(MethodParameter parameter) {
        boolean hasLoginUserAnnotation = parameter.hasParameterAnnotation(PowerSession.class);
