...
const UserManagement = ({handlersRef}: {onChange?: (flag: boolean) => void; menuInfo?: any; handlersRef?: React.MutableRefObject<PageButtonHandlers>}) => {
    ...
    const handleSearchList = async () => {
        ...
        callGetUserList({ userId: idOrName, userName: idOrName, isCheck }).then((res)=> {
    ...
    /** 사용시작일/사용종료일을 varchar(8) YYYYMMDD 형식으로 변환 */
    ...
    const handleSave = async (value: UserList) => {
        ...
        if(result) {
            const payload = {
             ...
             callSaveUser(payload).then((result)=>{
    ...
    const handleAdd = async() => {
        ...
        addList[0].strDate = dayjs().format('YYYY-MM-DD');
    ...
    const handleRowChanged = async(clickedRecord?: User, clickedIndex?: number) => {
        // 현재 선택된 행을 다시 클릭한 경우(생성 중인 행 포함) confirm 없이 진행
